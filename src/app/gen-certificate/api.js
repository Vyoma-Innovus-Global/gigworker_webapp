...
export async function uploadSendOtp(aadhaar) {
  try {
    const response = await fetch(`${API_BASE_URL}aadhaarSendOtp`, {
      method: "POST",
      headers: {
        accept: "*/*",
...
export async function uploadVerifyOtp(otp, transactionId) {
  try {
    const response = await fetch(`${API_BASE_URL}aadhaarVerifyOtp`, {
      method: "POST",
      headers: {
        accept: "*/*",
        Authorization: `Bearer ${TOKEN}`,
...
export const updateApplicationApprovalStatusAPI = async () => {
  try {
    const udin_token = Cookies.load("udin_token") || "";
    const aid = Cookies.load("aid");
    ...
    return await callApi("updateApplicationApprovalStatus", {
      application_id: aid,
      application_qr_image: application_qr_image,
      token_number: udin_token,
    });
