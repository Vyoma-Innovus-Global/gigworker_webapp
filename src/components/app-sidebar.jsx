...
import { getDashboardData } from "@/app/worker-dashboard/api"; // Adjust the import path as needed
...
export const AdminAppSidebar = (props) => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (res.status === 0) {
          ...
          setWorkerName(res?.data[0]?.gig_worker_name || "Worker");
        }
      } catch (error) {
  ...
  // Create user object with the fetched name
  const currentUser = {
    name: workerName,
    email: "worker@example.com",
    avatar: "/img/user.jpg",
