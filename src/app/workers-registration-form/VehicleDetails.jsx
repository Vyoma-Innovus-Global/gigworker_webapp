...
import { AlertCircle, PlusCircle, SquarePenIcon, Trash2 } from "lucide-react";
...
function VehicleDetails({ formData, setFormData, initialVehicles }) {
  ...
  const handleSelectChange = (field, value) => {
    ...
    if (field == "automobile_cycle_id") {
      if (value == 20) {
        setFormData((prev) => ({ ...prev, is_self_owned: 1, vehicle_type_id: 200, registration_type_id: 1, registration_number: "", driving_license: "" }));
      } else if (value == 30) {
        setFormData((prev) => ({ ...prev, is_self_owned: 1, vehicle_type_id: 200, registration_type_id: 1, registration_number: "", driving_license: "" }));
      } else {
        setFormData((prev) => ({ ...prev, is_self_owned: "", vehicle_type_id: "", registration_type_id: "", registration_number: "", driving_license: "" }));
      }
    };
  }
  ...
  return (
    <div className="p-4 border rounded">
      ...
      <div className="text-sm text-gray-500 border bg-violet-50 rounded p-4 mb-4 flex items-center gap-2 space-x-2">
        <AlertCircle className="text-violet-600" /> If your occupation does not require the use of a vehicle, you may proceed without filling in the Vehicle Details section by clicking the "Save & Next" button.
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        {/* Automobile/Cycle */}
        <div>
          <Label htmlFor="automobile_cycle_id">Automobile/Bicycle/Electric Bicycle</Label>
          <Select
            onValueChange={(value) =>
        ...
        {/* Self-owned */}
        <div>
          <Label htmlFor="is_self_owned">Nature of Ownership</Label>
          <Select
            onValueChange={(value) =>
        ...
        {/* Registered as */}
        <div>
          <Label htmlFor="registration_type_id">Registered as</Label>
          <Select
            onValueChange={(value) =>
              ...
              <table className="border-collapse w-full text-sm">
                <thead>
                  <tr>
                    ...
                    <th className="border text-left px-4 py-2">Registered as</th>
                    <th className="border text-left px-4 py-2">Automobile/ Bicycle/ Electric Bicycle</th>
                    <th className="border text-left px-4 py-2">Two/Four Wheelers</th>
                    <th className="border text-left px-4 py-2">Nature of Ownership</th>
                  ...
                  {formData?.arr_vehicle_details
                    ...
                    .map((vehicle, index) => (
                      <tr key={index}>
                        ...
                        <td className="border px-4 py-2 text-center">
                          {vehicle.is_self_owned === 1 ? "Own" : "Hired"}
                        </td>
                        <td className="border px-4 py-2 text-center">
                        ...
                        <td className="border px-4 py-2 text-center space-x-2">
                          <div className="flex justify-center space-x-2">
                            <button
                              type="button"
                              className="text-yellow-600 hover:text-yellow-900 rounded-full w-8 h-8 flex items-center justify-center hover:bg-yellow-500 border border-gray-300 bg-white"
                            ...
                            <button
                              type="button"
                              className="text-red-500 hover:text-slate-100 rounded-full w-8 h-8 flex items-center justify-center hover:bg-red-500 border border-gray-300 bg-white"
