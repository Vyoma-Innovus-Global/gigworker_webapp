...
export function DataTable({ columns, data: initialData }) {
    ...
    const handleExportToPDF = () => {
        try {
            ...
            setTimeout(() => {
                // 1. Create jsPDF instance with landscape orientation and A4 size
                const doc = new jsPDF({
                ...
                const tableRows = filteredData.map((row) =>
                    columns.map((col) =>
                        row[col.accessorKey] !== null && row[col.accessorKey] !== undefined
                            ? row[col.accessorKey]
                            : "N/A"
                    )
                );
                ...
                doc.save("report.pdf");
                setPdfLoading(false); // Moved here to ensure it's only called after save
            }, 1000); // Optional delay for any loading UI
        } catch (error) {
